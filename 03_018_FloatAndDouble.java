package com.timbuchalka;

public class Main {

    public static void main(String[] args) {
        // width of int = 32 (4 bytes).
        int myIntValue = 5 / 3;
        // width of float = 32 (4 bytes).
        float myFloatValue = 5f / 3f;
        // width of double = 64 (8 bytes)
        double myDoubleValue = 5d / 3d;
        System.out.println("myIntValue = " + myIntValue);       // 1
        System.out.println("myFloatValue = " + myFloatValue);   // 1.6666666
        System.out.println("myDoubleValue = " + myDoubleValue); // 1.6666666666666667

        // Convert a given number of pounds to kilograms
        // 1.  Create a variable to store the number of pounds
        double poundsValue = 200d;
        // 2. Calculate the number of Kilograms for the number above and store in a variable.
        double kilogramValue = poundsValue * 0.45359237;
        // 3. Print out the result.
        System.out.println(poundsValue + " lbs = " + kilogramValue +" kgs");  // 200.0 lbs = 90.718474 kgs

        double randomNum = 3_000_000.123_445d;

    }
}
